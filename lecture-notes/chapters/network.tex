...
\end{Code}
The by far most commonly used protocol is \texttt{https}, but \texttt{http} and \texttt{ftp} are also common but a url can contain any protocol your application and network stack can handle, for example Secure Shell (SSH) or Network Time Protocol (NTP). Note, a http-api, such as \code{window.fetch} can only handle http/https. 

The authority part of the example url is the DNS name of the server. An IP-number may also be used. If a port is not given, the default for the protocol is used, 443 for https and 80 for http. In the labs you use port 3000, which is used by the node.js development http server.
