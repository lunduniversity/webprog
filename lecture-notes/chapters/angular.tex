...
\end{Code}
It does two things: 1, declares a \code{public todoService} attribute 2, initialises the attribute to the value passed to the constructor, that is, it is shorthand for:
\begin{Code}
export class MyComponent implements OnInit {
...
\end{Code}
Note, \code{"special"} is a string, not an expression. 
...
Components can have input, use the \code{@Input} decoration to bind a class attribute to a input:
\begin{Code}
@Component({
