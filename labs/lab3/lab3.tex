...
\begin{itemize}
  ...
  \item Create a functional component: \code{ViewIngredient}. The ingredient name must be passed in the url path. Pass \code{inventory} to it as a prop, as you did with \code{ComposeSalad}.
  \item Add \code{<Route path='/view-ingredient/:name' ...>} to your router.
\end{itemize}

\noindent \emph{Reflection Question 3:} What is the meaning of a leading / in a path, the difference between \code{<Link to="/view-ingredient/:name" />} and \code{<Link to="view-ingredient/:name" />}. Try it, look in the browser url field.
