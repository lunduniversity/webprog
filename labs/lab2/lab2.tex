...
\item Create a component for composing salads. Pass \code{inventory} to it using \code{props}. I sugest you name it ComposeSalad:
\begin{Code}
...
\item Lets use the component, instansiate it in \code{App.js}:
\begin{Code}
...
\begin{itemize}
  ...
  \item Remember to bind your callback functions:\\ \code{this.handleChange = this.handleChange.bind(this);}
  ...
  \item For checkboxes, the state of the DOM-element is stored in the property named \code{checked} (for other \code{<input>} types \code{value} is used). Do not assign \code{undefined} ti it. To avoid this, you can use the JavaScript short cut behaviour of \code{||} \\ \code{<input checked=(this.state['Tomat']||false)>}.
  \item \code{<select>} and \code{<option>} might be a good alternative for selecting the foundation and dressing.
